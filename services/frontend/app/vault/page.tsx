          ...
          <div className="mb-6 flex items-start justify-between gap-4">
            ...
            {user && (
              <a
                ...
                className="shrink-0 flex items-center gap-1.5 border border-black/20 bg-white px-3 py-1.5 font-mono text-[10px] font-bold text-text-secondary transition-colors hover:border-black/40 hover:text-text-primary"
              >
                🔗 Share Profile
              </a>
            )}
              ...
              <div className="space-y-1">
                ...
                {resumesLoading ? (
                  ...
                  resumes.map((r) => {
                    ...
                    const isDeleting = deletingId === r.resumeId;
                    return (
                      <div
                        key={r.resumeId}
                        className={`
                      ...
                      >
                                ...
                                <span className={`border px-1.5 py-0.5 font-mono text-[9px] font-bold ${statusBadge(r.status)}`}>
                                  {r.status === 'PROCESSING' || r.status === 'UPLOADED' ? (
                                    <span className="flex items-center gap-1">
                                      <span className="animate-pulse">⏳</span> {r.status}
                                    </span>
                                  ) : r.status}
                                </span>
                                {hasAnalysis && (
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); handleDeleteResume(r.resumeId); }}
                          disabled={isDeleting}
                          title="Delete resume"
                          className="absolute right-1 top-1 hidden h-6 w-6 items-center justify-center text-text-tertiary opacity-0 transition-opacity hover:text-red-600 group-hover:flex group-hover:opacity-100"
                        >
